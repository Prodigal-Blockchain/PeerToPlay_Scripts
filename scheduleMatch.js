async function main() {
  ...
  const teamId = 1;
  const location = "Banagalore";
  const fee = 10;
  const startTime = 1716615482; //EPOC Timestamp https://www.epochconverter.com/
  const matchType = 1; //dummy variable for future use in setting any configurations
  //   const players = [1, 2];
  const encodedFunctionCall = connector.methods
    .scheduleMatch(teamId, location, fee, startTime, matchType)
    .encodeABI();
